...
app.post('/api/attendance', (req, res) => {
  try {
    ...
    if (!teacherName || !subject || !section || !date || !Array.isArray(students)) {
    ...
    const insertSession = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
    const sessionResult = insertSession.run(teacherName, subject, section, date, now);
...
app.post('/api/students', (req, res) => {
  try {
    ...
    if (!name || !rollNumber || !section) {
      return res.status(400).json({ error: 'name, rollNumber and section are required' });
    }
    ...
    if (existing) {
      return res.status(409).json({ error: 'Student already exists in this section' });
    }
